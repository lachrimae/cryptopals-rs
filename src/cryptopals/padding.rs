...
impl block_padding::Padding for NullPadding {
    fn pad_block(_block: &mut [u8], _pos: usize) -> Result<(), block_padding::PadError> {
        Ok(())
    }
    ...
    fn unpad(data: &[u8]) -> Result<&[u8], block_padding::UnpadError> {
        Ok(&data)
    }
}
...
pub fn depkcs7(block: &mut Vec<u8>) -> Result<(), String> {
    ...
    for &pad_char in pad_candidate.iter() {
        if pad_char != last_char {
            return Err(format!(
                "Bad padding! expected {} '{}' chars",
                last_char, last_char as char
            ));
        }
    }
